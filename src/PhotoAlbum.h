// Saves to a dedicated "CapsuleGram" album in the Photos library.
// Creates the album on first use. All CapsuleGram-initiated saves should go
...
// Asynchronously fetches (or creates on first use) the CapsuleGram album.
+ (void)fetchOrCreateAlbumWithCompletion:(void (^)(PHAssetCollection *album, NSError *error))completion;

// Saves a file at fileURL into the CapsuleGram album. The file is treated as a
...
// the CapsuleGram album. Used to capture saves performed via UIActivityViewController's
